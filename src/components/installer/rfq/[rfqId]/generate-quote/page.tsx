...
import { getMockUserById, type MockUser } from "@/lib/mock-data/users"; // Keep for RFQ details
...
import { FileSpreadsheet, PlusCircle, Trash2, Send, Info, UserCircle, Home, Edit, AlertTriangle } from "lucide-react";
...
export default function GenerateQuotePage() {
  ...
  const [installerProfile, setInstallerProfile] = React.useState<MockUser | null>(null); // Firestore profile
 ...
 React.useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        if (userDocSnap.exists() && userDocSnap.data()?.role === 'installer') {
          ...
          setInstallerProfile(null); // Not an installer or profile not found in Firestore
          toast({ title: "Access Denied", description: "You must be logged in as an installer.", variant: "destructive" });
        }
      } else {
        setInstallerProfile(null);
      }
      // Fetch RFQ details regardless of auth state for now, but UI will block if not installer
      const rfq = getRFQById(rfqId); // Using mock RFQ data
      if (rfq) {
        setRfqDetails(rfq);
        form.reset({
          ...form.getValues(), 
  ...
  const onSubmit: SubmitHandler<QuoteFormData> = async (data) => {
    if (!installerProfile) {
        toast({ title: "Error", description: "Installer profile not loaded.", variant: "destructive" });
        return;
    }
  ...
  if (!rfqDetails) {
    // This case should be handled by the redirect in useEffect if RFQ not found, but as a fallback:
    return (
        <div className="max-w-xl mx-auto text-center py-12">
