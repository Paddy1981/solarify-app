
"use client";

import { Bar, BarChart, CartesianGrid, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer } from "recharts";
import { ChartConfig, ChartContainer, ChartTooltipContent, ChartLegend, ChartLegendContent } from "@/components/ui/chart";
import type { MonthlySavingEntry } from '@/app/homeowner/savings-tracker/page';
import { format } from 'date-fns';

interface SavingsComparisonChartProps {
  data: MonthlySavingEntry[];
  currencySymbol?: string;
}

export function SavingsComparisonChart({ data, currencySymbol = "$" }: SavingsComparisonChartProps) {
 const chartConfig = {
    projectedSavings: {
      label: `Projected Savings (${currencySymbol})`,
      color: "hsl(var(--chart-2))",
    },
    actualSavings: {
      label: `Actual Savings (${currencySymbol})`,
      color: "hsl(var(--chart-1))",
    },
  } satisfies ChartConfig;

  const chartData = data.map(entry => ({
    month: format(new Date(entry.month), "MMM yy"),
    projectedSavings: entry.projectedSavings,
    actualSavings: entry.actualSavings,
  }));

  return (
    <ChartContainer config={chartConfig} className="min-h-[300px] w-full">
      <ResponsiveContainer width="100%" height={350}>
        <BarChart 
            data={chartData}
            margin={{
                top: 5,
                right: 20,
                left: 0,
                bottom: 5,
            }}
        >
          <CartesianGrid strokeDasharray="3 3" vertical={false} />
          <XAxis
            dataKey="month"
            tickLine={false}
            axisLine={false}
            tickMargin={10}
            // angle={-30}
            // textAnchor="end"
          />
          <YAxis 
            tickLine={false}
            axisLine={false}
            tickMargin={10}
            tickFormatter={(value) => `${currencySymbol}${value}`}
          />
          <Tooltip
            cursor={{ fill: 'hsl(var(--muted))' }}
            content={<ChartTooltipContent indicator="dot" hideLabel />}
          />
           <ChartLegend content={<ChartLegendContent />} />
          <Bar dataKey="projectedSavings" fill="var(--color-projectedSavings)" radius={[4, 4, 0, 0]} />
          <Bar dataKey="actualSavings" fill="var(--color-actualSavings)" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </ChartContainer>
  );
}
